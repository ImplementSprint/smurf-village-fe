...
function hasDefaultSupabase(env) {
    return Boolean(env.SUPABASE_URL &&
        env.SUPABASE_ANON_KEY &&
        env.SUPABASE_SERVICE_ROLE_KEY);
}
function hasScopedSupabase(env) {
...
function throwMissing(name) {
    throw new Error(`[env] Missing required environment variable: ${name}. Set it in .env or deployment secrets.`);
}
function validateEnv(input) {
    const env = { ...input };
    const nodeEnv = env.NODE_ENV ?? 'development';
    if (!VALID_NODE_ENVS.has(nodeEnv)) {
        throw new Error(`[env] NODE_ENV must be one of: development, test, production. Received: ${nodeEnv}`);
    }
    ...
    if (!hasTribeCreds && !hasLegacyKey) {
        throw new Error('[env] Provide APICenter tribe credentials (API_CENTER_TRIBE_ID + API_CENTER_TRIBE_SECRET) or API_CENTER_API_KEY.');
    }
    if (!hasDefaultSupabase(env) && !hasScopedSupabase(env)) {
        throw new Error('[env] Provide either default Supabase credentials (SUPABASE_URL, SUPABASE_ANON_KEY, SUPABASE_SERVICE_ROLE_KEY) or at least one scoped pair (<SERVICE>_SUPABASE_URL + <SERVICE>_SUPABASE_SECRET_KEY).');
    }
    return env;
}
