...
exports.toStartCaseName = toStartCaseName;
exports.normalizeNamePart = normalizeNamePart;
exports.isAtLeastAge = isAtLeastAge;
function toStartCaseName(value) {
    return value
        .trim()
        .replace(/\s+/g, ' ')
        .toLocaleLowerCase('en-US')
        .replace(/(^|[\s'-])([a-z])/g, (_match, prefix, letter) => `${prefix}${letter.toLocaleUpperCase('en-US')}`);
}
...
function isAtLeastAge(dateOfBirth, minimumAge, today = new Date()) {
    ...
    const birthdayThisYear = new Date(Date.UTC(reference.getUTCFullYear(), birthDate.getUTCMonth(), birthDate.getUTCDate()));
