...
const switch_role_dto_1 = require("./dto/switch-role.dto");
const COOKIE_NAME = 'refresh_token';
function setCookieOptions(maxAgeMs) {
...
let AuthController = class AuthController {
    ...
    async login(loginDto, req, res) {
        const { access_token, refresh_token, refresh_max_age_ms } = await this.authService.login(loginDto, req);
    ...
    async refresh(req, res) {
        ...
        if (!token)
            throw new common_1.UnauthorizedException('No refresh token cookie');
        const { access_token, refresh_token, refresh_max_age_ms } = await this.authService.refresh(token);
        res.cookie(COOKIE_NAME, refresh_token, setCookieOptions(refresh_max_age_ms));
        return { access_token };
    }
    ...
    switchRole(req, dto) {
        return this.authService.switchRole(req.user.sub_userid, req.user.company_id, dto.role_id);
    }
};
...
__decorate([
    ...
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, switch_role_dto_1.SwitchRoleDto]),
    __metadata("design:returntype", void 0)
