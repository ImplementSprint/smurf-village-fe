...
let PerformanceService = class PerformanceService {
    ...
    async getAllCycleIds(companyId) {
        const { data } = await this.db
            .from('performance_management_cycle')
            .select('perf_cycle_id')
            .eq('company_id', companyId);
        return (data || []).map((c) => c.perf_cycle_id);
    }
    async getEvalIdsForCycle(cycleId) {
        const { data } = await this.db
            .from('performance_evaluations')
            .select('perf_eval_id')
            .eq('perf_cycle_id', cycleId);
        return (data || []).map((e) => e.perf_eval_id);
    }
    ...
    actorRole(roleName) {
        if (['Active Employee', 'Employee'].includes(roleName))
            return 'EMPLOYEE';
        if (['Manager', 'Group Head'].includes(roleName))
            return 'MANAGER';
        if ([
            'HR Officer',
            'HR Recruiter',
            ...
            return 'HR';
        if (['System Admin', 'Admin'].includes(roleName))
            return 'SYSTEM_ADMIN';
        return roleName;
    }
    roundCurrency(value) {
    ...
    normalizePipStatus(status) {
        if (!status)
            return undefined;
        return status === 'FAILED' ? 'TERMINATED' : status;
    }
    async ensureRewardBenefitCatalog(companyId, rewardType) {
        if (rewardType === 'PROMOTION')
            return null;
        const benefitName = rewardType === 'MERIT_INCREASE' ? 'Performance Merit Increase' : 'Performance Bonus';
    ...
    async syncRewardIntoPayroll(rewardId) {
        const { data: reward, error: rewardError } = await this.db
            ...
            .maybeSingle();
        if (rewardError)
            throw new common_1.BadRequestException(rewardError.message);
        if (!reward)
        ...
        const { data: evaluation, error: evaluationError } = await this.db
            ...
            .maybeSingle();
        if (evaluationError)
            throw new common_1.BadRequestException(evaluationError.message);
        if (!evaluation)
            throw new common_1.NotFoundException('Evaluation not found for reward');
        const { data: cycle, error: cycleError } = await this.db
            ...
            .maybeSingle();
        if (cycleError)
            throw new common_1.BadRequestException(cycleError.message);
        if (!cycle?.company_id)
            throw new common_1.NotFoundException('Company not found for reward');
        if (reward.reward_type !== 'PROMOTION') {
            ...
            if (benefitId && amount > 0) {
                ...
                if (benefitError)
                    throw new common_1.BadRequestException(benefitError.message);
            }
            if (reward.reward_type === 'MERIT_INCREASE' && amount > 0) {
                const { data: currentBaseline, error: baselineReadErr } = await this.db
                    ...
                    .maybeSingle();
                if (baselineReadErr)
                    throw new common_1.BadRequestException(baselineReadErr.message);
                if (currentBaseline) {
                    const oldSalaryRaw = this.encryption.decryptToNumber(currentBaseline.basic_salary ?? '0');
                    ...
                    if (newBaselineErr)
                        throw new common_1.BadRequestException(newBaselineErr.message);
                    ...
                    if (auditErr)
                        throw new common_1.BadRequestException(auditErr.message);
                }
            }
        }
        const { data: synced, error: syncError } = await this.db
            ...
            .single();
        if (syncError)
            throw new common_1.BadRequestException(syncError.message);
        return synced;
    }
    calcRewardAmount(rule, baseSalary, type) {
    ...
    mapGoal(g) {
        ...
        if (g.status === 'PENDING_APPROVAL') {
            ...
            if ((g.progress_pct ?? 0) >= 100) {
                status = 'Achieved';
                statusColor = '#10b981';
            }
            else if ((g.progress_pct ?? 0) < 30) {
                status = 'At Risk';
                statusColor = '#ef4444';
            }
        }
    ...
    async getFullSettings(companyId) {
        ...
        const violRules = (violRulesRes.data || []).map((r) => ({
            ...r,
            id: r.rule_id,
        ...
        const bonusRules = (bonusRulesRes.data || []).map((r) => ({ ...r, id: r.bonus_rule_id }));
        return { settings: settingsRes.data, violation_rules: violRules, bonus_rules: bonusRules };
    }
    async saveFullSettings(companyId, userId, body) {
        const { settings, violation_rules, bonus_rules } = body;
        if (settings) {
            const { error } = await this.db.from('performance_cycle_settings').upsert({ ...settings, company_id: companyId, updated_by: userId }, { onConflict: 'company_id' });
            if (error)
                throw new common_1.BadRequestException(error.message);
        }
        if (violation_rules !== undefined) {
            ...
            if (violation_rules.length > 0) {
                const rows = violation_rules.map((r) => ({ ...r, company_id: companyId, is_active: true }));
    ...
    async createViolationRule(companyId, dto) {
        ...
        if (!violation_count && condition) {
            ...
            if (m) {
                violation_count = parseInt(m[1]);
                severity_threshold = m[2];
            }
        }
        if (!resulting_action && action)
            resulting_action = action.toUpperCase().replace(/\s+/g, '_');
    ...
    async deleteViolationRule(ruleId, companyId) {
    ...
    async deleteBonusRule(ruleId, companyId) {
    ...
    async getActiveCyclePublic(companyId) {
        const cycle = await this.getActiveCycle(companyId);
        if (!cycle)
            throw new common_1.NotFoundException('No active performance cycle found');
        return cycle;
    }
    async createCycle(companyId, userId, dto) {
    ...
    async patchCycle(cycleId, companyId, dto) {
    ...
    async deleteCycle(cycleId, companyId) {
        ...
        if ((goalsRes.count || 0) > 0 || (evalsRes.count || 0) > 0) {
            throw new common_1.BadRequestException('Cannot delete a cycle that has associated goals or evaluations. Mark it as COMPLETED instead.');
        }
    ...
    async getMyGoals(userId, companyId) {
        const cycle = await this.getActiveCycle(companyId);
        if (!cycle)
            return [];
    ...
    async getTeamGoals(managerId, companyId) {
        const cycle = await this.getActiveCycle(companyId);
        if (!cycle)
            return [];
    ...
    async createGoal(requesterId, requesterRole, companyId, dto) {
        ...
        if (isEmployee && settings && settings.self_proposed_goals_enabled === false) {
            throw new common_1.ForbiddenException('Self-proposed goals are disabled.');
        }
        const cycle = await this.getActiveCycle(companyId);
        if (!cycle)
            throw new common_1.BadRequestException('No active performance cycle found.');
    ...
    async patchGoal(goalId, requesterId, companyId, dto) {
    ...
    async deleteGoal(goalId, requesterId, companyId) {
        ...
        if (goal.status === 'APPROVED')
            throw new common_1.BadRequestException('Cannot delete an approved goal. Reject it first via HR.');
        const { error } = await this.db.from('performance_goals').delete().eq('perf_goals_id', goalId);
        if (error)
            throw new common_1.BadRequestException(error.message);
        return { message: 'Goal deleted' };
    }
    async approveGoal(goalId, approverId, companyId) {
        const { data, error } = await this.db.from('performance_goals').update({
    ...
    async rejectGoal(goalId, approverId, companyId, reason) {
        const { data, error } = await this.db.from('performance_goals').update({
    ...
    async addGoalProgress(goalId, userId, companyId, dto) {
        if (dto.progress_pct < 0 || dto.progress_pct > 100)
            throw new common_1.BadRequestException('progress_pct must be between 0 and 100');
    ...
    async buildGoalsChecklist(userId, cycleId) {
        ...
        return (goals || []).map((g) => ({ label: g.goal_name, checked: (g.progress_pct || 0) >= 100 }));
    }
    async getMyEvaluations(userId, companyId) {
        const cycle = await this.getActiveCycle(companyId);
        if (!cycle)
            return [];
    ...
    async getMyEvaluationHistory(userId, companyId) {
    ...
    async getTeamEvaluations(managerId, companyId) {
        const cycle = await this.getActiveCycle(companyId);
        if (!cycle)
            return [];
    ...
    async getAllEvaluations(companyId) {
        const cycle = await this.getActiveCycle(companyId);
        if (!cycle)
            return [];
    ...
    async createEvaluation(reviewerId, reviewerRole, companyId, dto) {
        const cycle = await this.getActiveCycle(companyId);
        if (!cycle)
            throw new common_1.BadRequestException('No active performance cycle');
        ...
        if (cycleSettings?.auto_compute_bonuses && !(dto.recommendations?.bonus || dto.recommendations?.merit) && sharedRule) {
            ...
            if (bonusAmt > 0)
                await this.db.from('performance_rewards').insert({ perf_eval_id: data.perf_eval_id, reward_type: 'BONUS', amount_value: bonusAmt, is_synced: false });
            if (meritAmt > 0)
                await this.db.from('performance_rewards').insert({ perf_eval_id: data.perf_eval_id, reward_type: 'MERIT_INCREASE', amount_value: meritAmt, is_synced: false });
        }
    ...
    async submitEvaluation(evalId, reviewerId, companyId) {
    ...
    async countersignEvaluation(evalId, hrId, companyId) {
    ...
    async acknowledgeEvaluation(evalId, userId) {
    ...
    async getViolationsDetailed(companyId) {
        const SUGGESTED = { LOW: 'Verbal Warning', MEDIUM: 'Written Warning', HIGH: 'Suspension', CRITICAL: 'Escalation / Termination' };
    ...
    async getViolationStats(companyId) {
    ...
    async createViolation(loggedBy, loggerRole, companyId, dto) {
        ...
        for (const rule of (rules || [])) {
            ...
            if (withinClause)
                countQuery = countQuery.gte('occured_at', withinClause);
    ...
    async patchViolation(violId, companyId, dto) {
    ...
    async uploadViolationEvidence(file, companyId) {
    ...
    async uploadPerformanceDocument(file, companyId) {
        const ext = file.originalname.split('.').pop() ?? 'bin';
    ...
    async createPip(initiatorId, initiatorRole, companyId, dto) {
        const cycle = await this.getActiveCycle(companyId);
        ...
        if ((count || 0) >= maxAttempts)
            throw new common_1.BadRequestException(`Maximum PIP attempts (${maxAttempts}) reached for this employee.`);
    ...
    async approvePip(pipId, approverId, companyId) {
    ...
    async patchPipStatus(pipId, dto) {
        const payload = {
            ...dto,
    ...
    async createPipUpdate(pipId, userId, dto) {
    ...
    async reviewPipUpdate(updateId, reviewerId, dto) {
    ...
    async getRewards(companyId) {
        ...
        if (!evals || evals.length === 0)
            return [];
        const evalIds = evals.map((e) => e.perf_eval_id);
        const evalMap = {};
        for (const e of evals)
            evalMap[e.perf_eval_id] = e;
        ...
        if (error)
            throw new common_1.BadRequestException(error.message);
        const uniqueUserIds = [...new Set((rewards || []).map((r) => evalMap[r.perf_eval_id]?.user_id).filter(Boolean))];
        ...
        for (const u of (usersRes.data || []))
            userNameMap[u.user_id] = `${u.first_name} ${u.last_name}`;
        const salaryMap = {};
        for (const s of (salariesRes.data || [])) {
            if (!salaryMap[s.user_id])
                salaryMap[s.user_id] = parseFloat(String(s.basic_salary || '0')) || 0;
        }
        const result = [];
        for (const r of (rewards || [])) {
            ...
            const status = ev.perf_status === 'Approved' ? (r.is_synced ? 'Pushed' : 'Approved') : 'Pending';
            result.push({
    ...
    async syncReward(rewardId) {
        return this.syncRewardIntoPayroll(rewardId);
    }
    async syncAllRewards(companyId) {
        const { data: cycles } = await this.db.from('performance_management_cycle').select('perf_cycle_id').eq('company_id', companyId);
        const cycleIds = (cycles || []).map((c) => c.perf_cycle_id);
        if (cycleIds.length === 0)
        ...
        const evalIds = (evals || []).map((e) => e.perf_eval_id);
        if (evalIds.length === 0)
            return { message: 'No rewards to sync' };
    ...
    async computeBonusRules(companyId, userId, rating) {
        const [ruleRes, salaryRes] = await Promise.all([
    ...
    async getEmployeeDashboard(userId, companyId) {
        const cycle = await this.getActiveCycle(companyId);
        if (!cycle)
            return { overall_progress_pct: 0, cycle_stage: 'GOAL_SETTING', active_goals_count: 0, achieved_count: 0, days_to_year_end: 0, midyear_rating: null, midyear_rating_label: null };
        ...
        const achieved = goalList.filter((g) => (g.progress_pct || 0) >= 100).length;
        const avgPct = active > 0 ? Math.round(goalList.reduce((s, g) => s + (g.progress_pct || 0), 0) / active) : 0;
    ...
    async getManagerDashboard(managerId, companyId) {
        const cycle = await this.getActiveCycle(companyId);
        const { data: team } = await this.db.from('user_profile').select('user_id').eq('company_id', companyId).neq('user_id', managerId);
        const teamList = team || [];
        const teamCount = teamList.length;
        if (teamCount === 0)
            return { team_count: 0, team_avg_rating: 0, on_track_count: 0, at_risk_count: 0, active_pip_count: 0, exceeding_count: 0 };
        const ids = teamList.map((u) => u.user_id);
        ...
        const pipUsers = new Set(pips.map((p) => p.user_id));
        const avgRatings = {};
        for (const e of evals) {
            if (!avgRatings[e.user_id])
                avgRatings[e.user_id] = [];
            avgRatings[e.user_id].push(e.scale_rating);
        }
        const avgProgress = {};
        for (const g of goalsList) {
            if (!avgProgress[g.user_id])
                avgProgress[g.user_id] = [];
        ...
        for (const u of ids) {
            ...
            const avgProg = progs.length > 0 ? progs.reduce((a, b) => a + b, 0) / progs.length : 0;
            if (avgRating >= 4.5)
                exceeding++;
            else if (avgProg < 40)
                atRisk++;
        ...
        const allRatings = evals.map((e) => e.scale_rating);
        const teamAvg = allRatings.length > 0 ? +(allRatings.reduce((a, b) => a + b, 0) / allRatings.length).toFixed(1) : 0;
    ...
    async getManagerTeam(managerId, companyId) {
        const cycle = await this.getActiveCycle(companyId);
        ...
        for (const m of members) {
            ...
            const avgProg = goals > 0 ? goalList.reduce((s, g) => s + (g.progress_pct || 0), 0) / goals : 0;
            let status = 'On Track';
            if (hasPip)
                status = 'PIP';
            else if (evalRes.data?.scale_rating >= 4.5)
                status = 'Exceeding';
            else if (avgProg < 40)
                status = 'At Risk';
    ...
    async getHrDashboard(companyId) {
        ...
        const cycleIds = (cyclesRes.data || []).map((c) => c.perf_cycle_id);
        let pending = 0;
        let evalIds = [];
        if (cycleIds.length > 0) {
            ...
            evalIds = (evalRes.data || []).map((e) => e.perf_eval_id);
        ...
        const rewards = evalIds.length > 0
            ...
            : [];
        const unsyncedRewards = rewards.filter((r) => !r.is_synced);
        const bonusTotal = unsyncedRewards.reduce((s, r) => s + (parseFloat(String(r.amount_value || '0')) || 0), 0);
        let passRate = 0;
        if (evalIds.length > 0) {
            ...
            passRate = allEvals.length > 0 ? Math.round(allEvals.filter((e) => e.rating_status === 'PASS').length / allEvals.length * 100) : 0;
        }
        const activeCycle = (cyclesRes.data || []).find((c) => c.cycle_name) || cyclesRes.data?.[0];
    ...
    async getHrApprovals(companyId, tab) {
        const cycle = await this.getActiveCycle(companyId);
        ...
        for (const u of (companyUsers || []))
            userNameMap[u.user_id] = `${u.first_name} ${u.last_name}`;
        const lookupName = (id) => userNameMap[id] || '';
        const items = [];
        ...
        if (['all', 'bonuses'].includes(tabLower)) {
            ...
            if (cycleIds.length > 0) {
                ...
                if (evalIds.length > 0) {
                    const evalUserMap = {};
                    for (const e of (evals || []))
                        evalUserMap[e.perf_eval_id] = e.user_id;
    ...
    async approveHrItem(itemId, approverId, dto) {
        const type = dto.type;
        if (type === 'Goal') {
        ...
        throw new common_1.BadRequestException(`Unknown approval type: ${type}`);
    }
    async reviewHrItem(itemId, approverId, dto) {
        ...
        throw new common_1.BadRequestException('Reject action not supported for this type');
    }
    async getActivityLogs(companyId, page = 1, limit = 50) {
    ...
    async getMySelfAssessment(userId, companyId) {
        const cycle = await this.getActiveCycle(companyId);
        if (!cycle)
            return null;
    ...
    async createOrUpdateSelfAssessment(userId, companyId, dto) {
        const cycle = await this.getActiveCycle(companyId);
        if (!cycle)
            throw new common_1.BadRequestException('No active performance cycle');
    ...
    async getSelfAssessmentByUser(targetUserId, companyId) {
        const cycle = await this.getActiveCycle(companyId);
        if (!cycle)
            return null;
        const { data, error } = await this.db.from('performance_self_assessments')
    ...
    async getRatingLabels(companyId) {
        ...
        if (!data)
            return ['Below Exp.', 'Below Exp.', 'Meets Exp.', 'Above Avg.', 'Excellent'];
