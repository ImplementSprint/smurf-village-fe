...
let LeaveService = LeaveService_1 = class LeaveService {
    ...
    logger = new common_1.Logger(LeaveService_1.name);
    constructor(supabaseService, mailService, leaveBalancesService) {
        this.supabaseService = supabaseService;
        this.mailService = mailService;
        this.leaveBalancesService = leaveBalancesService;
    }
    async getMyLeaveBalances(userId, companyId) {
        const my = await this.leaveBalancesService.getEmployeeBalances(userId, companyId);
        const categories = (my?.categories ?? []);
        const byCategory = new Map(categories.map((row) => [row.leave_category, row]));
        return [
            'Vacation Leave',
            'Sick Leave',
            'Emergency Leave',
    ...
    async fileLeaveRequest(userId, companyId, dto) {
        ...
        if (end < start) {
            throw new common_1.BadRequestException('end_date must be on or after start_date.');
        }
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const isRetro = dto.is_retro === true;
        if (isRetro) {
            if (!file_leave_request_dto_1.RETRO_ELIGIBLE_LEAVE_TYPES.includes(dto.leave_type)) {
                throw new common_1.BadRequestException(`Retro-filing is only allowed for: ${file_leave_request_dto_1.RETRO_ELIGIBLE_LEAVE_TYPES.join(', ')}. Use a standard request for ${dto.leave_type}.`);
            }
            if (!dto.retro_reason) {
                throw new common_1.BadRequestException('retro_reason is required when filing a retro leave request.');
            }
            if (start >= today) {
                throw new common_1.BadRequestException('Retro leave requests must have a start_date in the past.');
            }
        }
        else {
            if (start < today && dto.leave_type === 'Sick Leave') {
                throw new common_1.BadRequestException('Sick leave for past dates must use retro-filing (set is_retro: true and provide retro_reason).');
            }
        }
        ...
        if (overlapping && overlapping.length > 0) {
            ...
            throw new common_1.BadRequestException(`You already have a ${label} ${c.leave_type} request from ${c.start_date} to ${c.end_date}. Please revoke or wait for it to be resolved before filing another.`);
        }
        const effective = (await this.leaveBalancesService.getEmployeeBalances(userId, companyId));
        const categoryRow = (effective.categories ?? []).find((c) => c.leave_category === dto.leave_type);
        const allocated = Number(categoryRow?.entitled_days ?? 0);
        const used = Number(categoryRow?.used_days ?? 0);
        const remaining = allocated - used;
        if (totalDays > remaining) {
            throw new common_1.BadRequestException(`Insufficient ${dto.leave_type} balance. Remaining: ${remaining} day(s), requested: ${totalDays}.`);
        }
        ...
        this.logger.log(`Leave request filed — user: ${userId}, type: ${dto.leave_type}, days: ${totalDays}`);
        return data;
    }
    ...
    async getLeaveRequests(companyId, status) {
        const supabase = this.supabaseService.getClient();
        let query = supabase
            .from('time_leave_requests')
            .select('*')
    ...
    async reviewLeaveRequest(requestId, reviewerId, companyId, dto) {
        const supabase = this.supabaseService.getClient();
        ...
        if (dto.status === 'Approved') {
            await this.deductLeaveBalance(request.user_id, companyId, request.leave_type, Number(request.total_days), new Date(request.start_date).getFullYear());
            const todayPH = new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Manila' });
            if (request.end_date <= todayPH) {
                await this.backfillAttendanceForLeave(request.user_id, request.start_date, request.end_date, request.leave_type, reviewerId);
            }
        }
        this.logger.log(`Leave request ${dto.status} — request: ${requestId}, reviewer: ${reviewerId}`);
        void (async () => {
            try {
                ...
                this.logger.warn(`Leave review email failed: ${e.message}`);
            }
        })();
        return updated;
    }
    async cancelPendingLeave(requestId, userId) {
        const supabase = this.supabaseService.getClient();
        const { data: request, error: fetchErr } = await supabase
            .from('time_leave_requests')
            .select('*')
        ...
        if (!request)
            throw new common_1.NotFoundException('Leave request not found.');
        if (request.status !== 'Pending') {
            throw new common_1.BadRequestException('Only pending requests can be cancelled directly.');
        }
    ...
    async requestLeaveRevocation(requestId, userId, reason) {
        const supabase = this.supabaseService.getClient();
        const { data: request, error: fetchErr } = await supabase
            .from('time_leave_requests')
            .select('*')
        ...
        if (request.status !== 'Approved') {
            throw new common_1.BadRequestException('Only approved leave requests can have revocation requested.');
        }
        const todayPH = new Date(new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Manila' }));
        const startDate = new Date(request.start_date);
        if (startDate <= todayPH) {
            throw new common_1.BadRequestException('Revocation can only be requested for future leave dates. Past or ongoing leave cannot be revoked.');
        }
        if (!reason || reason.trim().length === 0) {
            throw new common_1.BadRequestException('A reason is required to request leave revocation.');
        }
        const { error: updateErr } = await supabase
            .from('time_leave_requests')
            .update({
    ...
    async reviewLeaveRevocation(requestId, reviewerId, companyId, action) {
        const supabase = this.supabaseService.getClient();
        const { data: request, error: fetchErr } = await supabase
            .from('time_leave_requests')
            .select('*')
        ...
        if (request.status !== 'RevocationRequested') {
            throw new common_1.BadRequestException('This request is not pending revocation review.');
        }
        if (action === 'approve') {
            await supabase
                .from('time_leave_requests')
                ...
                .eq('request_id', requestId);
            await this.refundLeaveBalance(request.user_id, companyId, request.leave_type, Number(request.total_days), new Date(request.start_date).getFullYear());
    ...
    async backfillAttendanceForLeave(userId, startDate, endDate, leaveType, reviewerId) {
        const supabase = this.supabaseService.getClient();
        ...
        if (error) {
            ...
            this.logger.log(`Attendance backfilled — employee: ${profile.employee_id}, range: ${startDate}–${endDate}, type: ${leaveType}`);
        }
    }
    async deductLeaveBalance(userId, companyId, leaveType, days, year) {
        const supabase = this.supabaseService.getClient();
    ...
    async refundLeaveBalance(userId, companyId, leaveType, days, year) {
        const supabase = this.supabaseService.getClient();
        const { data: existing } = await supabase
            .from('time_leave_balances')
    ...
    async uploadLeaveAttachment(userId, file) {
        if (!file)
            throw new common_1.BadRequestException('No file uploaded.');
        const allowedTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/webp', 'application/pdf'];
        if (!allowedTypes.includes(file.mimetype)) {
            throw new common_1.BadRequestException('Invalid file type. Only JPG, PNG, WebP, or PDF allowed.');
        }
    ...
    async seedLeaveBalancesForNewEmployee(userId, companyId) {
        const supabase = this.supabaseService.getClient();
        const year = new Date().getFullYear();
        const rows = Object.entries(DEFAULT_LEAVE_ALLOCATIONS).map(([leaveType, allocated]) => ({
            balance_id: crypto.randomUUID(),
            user_id: userId,
            company_id: companyId,
            leave_type: leaveType,
        ...
        if (error) {
            this.logger.warn(`Failed to seed leave balances for ${userId}: ${error.message}`);
        }
    }
};
