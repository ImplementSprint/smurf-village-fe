"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ReviewLeaveRequestDto = void 0;
const class_transformer_1 = require("class-transformer");
const class_validator_1 = require("class-validator");
class ReviewLeaveRequestDto {
    status;
    rejection_reason;
}
exports.ReviewLeaveRequestDto = ReviewLeaveRequestDto;
__decorate([
    (0, class_validator_1.IsString)({ message: 'status must be a string' }),
    (0, class_transformer_1.Transform)(({ value }) => typeof value === 'string'
        ? value.charAt(0).toUpperCase() + value.slice(1).toLowerCase()
        : value),
    (0, class_validator_1.IsIn)(['Approved', 'Rejected'], {
        message: 'status must be either Approved or Rejected',
    }),
    (0, class_validator_1.IsNotEmpty)({ message: 'status is required' }),
    __metadata("design:type", String)
], ReviewLeaveRequestDto.prototype, "status", void 0);
__decorate([
    (0, class_validator_1.IsOptional)(),
    (0, class_validator_1.IsString)({ message: 'rejection_reason must be a string' }),
    (0, class_validator_1.MaxLength)(500, { message: 'rejection_reason must not exceed 500 characters' }),
    __metadata("design:type", String)
], ReviewLeaveRequestDto.prototype, "rejection_reason", void 0);
//# sourceMappingURL=review-leave-request.dto.js.map