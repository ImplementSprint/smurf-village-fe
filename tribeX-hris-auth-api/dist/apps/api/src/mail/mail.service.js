...
function emailWrapper(headerHtml, bodyHtml, footerExtra = '') {
    return `
<!DOCTYPE html>
<html lang="en">
<head>
...
let MailService = MailService_1 = class MailService {
    ...
    constructor(config, apiCenterSdk) {
        this.config = config;
        this.apiCenterSdk = apiCenterSdk;
    }
    ...
    async sendInvite(to, inviteLink) {
        try {
    ...
    async sendRegistrationConfirmation(to, companyName) {
        try {
            const header = brandHeader('Registration Received', "We've received your company registration");
            const body = `
    ...
    async sendPaymentConfirmation(to, companyName, plan) {
        try {
            const appUrl = this.config.get('APP_URL') ?? 'http://localhost:3000';
            const header = brandHeader('Payment Confirmed', 'Your subscription is now active', STATUS.success.headerBg);
    ...
    async sendSystemAdminCredentials(to, inviteLink) {
        try {
            const header = brandHeader('System Admin Account Created', 'Set your password to get started');
            const body = `
        ...
        ${noteCard('Security Notice', 'This link expires in 48 hours. Do not share it with anyone. Each link can only be used once.', STATUS.warning.bg, STATUS.warning.border, STATUS.warning.text)}
    ...
    async sendVerificationEmail(to, verifyLink) {
    ...
    async sendInterviewScheduleEmail(opts) {
        ...
        const rescheduleNotice = opts.isReschedule
            ? noteCard('Schedule Updated', 'Your interview has been rescheduled by HR. The new details are shown below — please update your calendar.', STATUS.warning.bg, STATUS.warning.border, STATUS.warning.text)
            : '';
    ...
    async sendApplicationStatusEmail(opts) {
        const stageCfg = {
            screening: {
                subject: `Application Under Review – ${opts.jobTitle}`,
                headerTitle: 'Application Under Review',
                message: `Your application for <strong>${opts.jobTitle}</strong> is now under review by our team. We will be in touch with next steps soon.`,
            },
            first_interview: {
                subject: `Selected for 1st Interview – ${opts.jobTitle}`,
                headerTitle: 'Moving to 1st Interview',
                message: `Congratulations! You have been selected to move forward to the <strong>1st Interview</strong> stage for <strong>${opts.jobTitle}</strong>. Watch for a follow-up email with your schedule details.`,
            },
            technical_interview: {
                subject: `Selected for Technical Interview – ${opts.jobTitle}`,
                headerTitle: 'Moving to Technical Interview',
                message: `Great work! You have advanced to the <strong>Technical Interview</strong> stage for <strong>${opts.jobTitle}</strong>. Watch for a follow-up email with your schedule details.`,
            },
            final_interview: {
                subject: `Selected for Final Interview – ${opts.jobTitle}`,
                headerTitle: 'Moving to Final Interview',
                message: `Excellent progress! You have been selected for the <strong>Final Interview</strong> stage for <strong>${opts.jobTitle}</strong>. Watch for a follow-up email with your schedule details.`,
            },
            hired: {
                subject: `Offer Extended – You're Hired! – ${opts.jobTitle}`,
                headerTitle: "Welcome to the Team!",
                message: `🎉 Congratulations, <strong>${opts.applicantName}</strong>! We are thrilled to extend you an offer for the position of <strong>${opts.jobTitle}</strong>. Please open your onboarding portal to accept your offer and begin the onboarding process.`,
            },
            offer_expired: {
        ...
        const cfg = stageCfg[opts.status];
        if (!cfg)
            return;
        const header = brandHeader(cfg.headerTitle, `Application for ${opts.jobTitle}`);
        const body = `
      ${bodyText(`Hi <strong>${opts.applicantName}</strong>,`)}
      ${bodyText(cfg.message, '12px')}
      ${divider()}
      ${bodyText('Log in to your applicant portal to track your application status.', '0')}`;
        await this.sendMail({
            to: opts.to,
            subject: cfg.subject,
            html: emailWrapper(header, body),
    ...
    async sendInterviewConfirmedEmail(opts) {
        const fmtDate = new Date(`${opts.scheduledDate}T${opts.scheduledTime}`).toLocaleDateString('en-US', {
            weekday: 'long', year: 'numeric', month: 'long', day: 'numeric',
        });
        const fmtTime = new Date(`2000-01-01T${opts.scheduledTime}`).toLocaleTimeString('en-US', {
        ...
        const header = brandHeader('Interview Confirmed', `${opts.stageLabel} for ${opts.jobTitle}`);
        const body = `
      ${bodyText(`Hi <strong>${opts.applicantName}</strong>, your confirmation for the <strong>${opts.stageLabel}</strong> interview for <strong>${opts.jobTitle}</strong> has been received.`)}

      <div style="background:${STATUS.success.bg};border:1px solid ${STATUS.success.border};border-radius:12px;padding:20px 24px;margin:20px 0;">
        <p style="margin:0 0 4px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:${STATUS.success.badge};font-family:'Open Sans',sans-serif;">Confirmed</p>
        <p style="margin:0;font-size:16px;font-weight:600;color:${STATUS.success.text};font-family:'Poppins',sans-serif;">${fmtDate} &middot; ${fmtTime}</p>
      </div>

      ${divider()}
      ${bodyText('We look forward to speaking with you. Log in to your applicant portal if you need to view or update your response.', '0')}`;
        await this.sendMail({
            to: opts.to,
            subject: `Interview Confirmed – ${opts.jobTitle}`,
            html: emailWrapper(header, body),
        });
    }
    async sendOnboardingItemReviewedEmail(opts) {
        ...
        const header = brandHeader(`Onboarding Item ${statusLabel}`, 'Your submitted document has been reviewed', st.headerBg);
        const body = `
    ...
    async sendOnboardingApprovedEmail(opts) {
        const header = brandHeader('Welcome to the team!', 'Your onboarding has been fully approved', STATUS.success.headerBg);
        const body = `
    ...
    async sendOnboardingRejectedEmail(opts) {
        const header = brandHeader('Onboarding Needs Revisions', 'Your final onboarding review was returned for updates', STATUS.warning.headerBg);
        const body = `
    ...
    async sendProfileChangeReviewedEmail(opts) {
        ...
        const header = brandHeader(`${fieldLabel} Change ${statusLabel}`, 'Your profile update request has been reviewed', st.headerBg);
        const body = `
    ...
    async sendAbsenceReviewEmail(opts) {
        ...
        const header = brandHeader(`Absence Request ${actionLabel}`, 'Your absence request has been reviewed', st.headerBg);
        const body = `
    ...
    async sendLeaveReviewEmail(opts) {
        ...
        const header = brandHeader(`Leave Request ${actionLabel}`, 'Your leave request has been reviewed by HR', st.headerBg);
        const body = `
    ...
    async sendOvertimeReviewEmail(opts) {
        ...
        const header = brandHeader(`Overtime Request ${actionLabel}`, 'Your overtime request has been reviewed by HR', st.headerBg);
        const body = `
    ...
    async sendRenewalReminder(to, companyName, daysRemaining, plan) {
        ...
        const header = emailWrapper(`<div style="background:${STATUS.warning.headerBg};padding:32px 40px 28px;">
      ...
      </div>`, `<p style="font-size:15px;color:${BRAND.textBody};margin:0 0 16px;">${urgency}</p>
    ...
    async sendSuspensionNotice(to, companyName) {
        const header = emailWrapper(`<div style="background:${STATUS.danger.headerBg};padding:32px 40px 28px;">
        ...
        <div style="color:rgba(255,255,255,0.7);font-size:14px;">Blues Clues HRIS</div>
      </div>`, `<p style="font-size:15px;color:${BRAND.textBody};margin:0 0 16px;">
