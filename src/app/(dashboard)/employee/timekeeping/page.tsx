...
function CalendarDayModal({
  ...
  const absenceReasonValue = entry?.absence?.absence_reason;
  const absenceReasonCfg = absenceReasonValue
    ? ABSENCE_REASONS.find(r => r.value === absenceReasonValue)
    : null;
  const headerClass = cfg ? `${cfg.cell} border-b` : "border-b border-border";
  const absenceReasonClass = absenceReasonCfg
    ? `${absenceReasonCfg.bg} ${absenceReasonCfg.border}`
    : "bg-purple-50 border-purple-200";

  let bodyContent: React.ReactNode;
  if (isFuture) {
    bodyContent = <p className="text-sm text-muted-foreground text-center py-4">No records yet - this is a future date.</p>;
  } else if (!entry || (!entry.time_in && !entry.absence)) {
    bodyContent = (
  ...
  } else if ((status === "excused" || status === "absent") && entry.absence) {
    bodyContent = (
      <div className="space-y-3">
        <div className={`flex items-center gap-3 p-3 rounded-xl border ${absenceReasonClass}`}>
          {absenceReasonCfg && (
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="relative bg-white rounded-2xl shadow-2xl border border-border w-full max-w-sm mx-4 animate-in zoom-in-95 duration-200 overflow-hidden">
        {/* Header */}
        <div className={`px-6 pt-6 pb-4 ${headerClass}`}>
        ...
        {/* Body */}
        <div className="p-6 space-y-4">{bodyContent}</div>
      </div>
    </div>
  );
}
...
function AbsenceModal({
  ...
  const selectedCfg = ABSENCE_REASONS.find(r => r.value === absenceReason) ?? ABSENCE_REASONS[0];
...
function getHistoryBody(
  ...
  if (sheetLoading) {
    return <p className="px-6 py-10 text-center text-muted-foreground text-sm">Loading records...</p>;
  }
  if (fetchError) {
    return <p className="px-6 py-10 text-center text-destructive text-sm">Failed to load records. Please refresh or contact support.</p>;
  }
  return view === "calendar" ? calendarView : listView;
}

function getActionSection(params: {
...
function getNextMonth(current: { year: number; month: number }): { year: number; month: number } {
  if (current.month === 11) {
...
export default function EmployeeTimekeepingPage() {
  ...
  useEffect(() => {
    const parseJsonOrThrow = async <T,>(response: Response, message: string): Promise<T> => {
      if (!response.ok) {
    ...
    authFetch(`${API_BASE_URL}/timekeeping/my-status`)
      .then((r) => parseJsonOrThrow<MyStatus>(r, "Failed to load attendance status."))
      .then(setStatus)
      .catch(() => setFetchError(true))
      .finally(() => setStatusLoading(false));

    authFetch(`${API_BASE_URL}/timekeeping/my-timesheet`)
      .then((r) => parseJsonOrThrow<TimesheetEntry[]>(r, "Failed to load timesheet records."))
      .then(setTimesheet)
      .catch(() => setFetchError(true))
  ...
  const historyBody = getHistoryBody(sheetLoading, fetchError, view, calendarView, listView);

  const actionSection = getActionSection({
    hasReportedAbsence,
    shiftDone,
    canTimeIn,
    canTimeOut,
    actionLoading,
    onTimeIn: () => { setActionError(null); setModal("time-in"); },
    onTimeOut: () => { setActionError(null); setModal("time-out"); },
  });
