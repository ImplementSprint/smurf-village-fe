...
function ApplicationDetailModal({
  ...
  const moveTo = async (targetStatus: string, silent = false, deadlineDays?: number) => {
    ...
    try {
      ...
      if (targetStatus === 'hired' && deadlineDays !== undefined) {
...
function ApplicantsModal({
  ...
  useEffect(() => {
    apiFetch<Application[]>(`/jobs/${job.job_posting_id}/applications${showPast ? '?include_past=true' : ''}`)
      .then(setApplications)
      .catch((err: any) => toast.error(err.message || "Failed to load applications"))
      .finally(() => setLoading(false));
  }, [job.job_posting_id, showPast]);
