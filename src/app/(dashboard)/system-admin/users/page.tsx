...
function DeptManageSheet({
  ...
  const membersBody = (() => {
    if (otherEmployees.length === 0) {
      return <p className="text-xs text-muted-foreground italic">All employees are already in this department.</p>;
    }

    if (filteredOther.length === 0) {
      return <p className="text-xs text-muted-foreground italic">No employees match your search.</p>;
    }
