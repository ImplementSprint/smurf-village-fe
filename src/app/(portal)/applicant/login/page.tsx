...
function ApplicantPortalAuthInner() {
  ...
  useEffect(() => {
    ...
    if (token && userInfo?.role === "applicant") {
  ...
  const submitLabel = isSignUp ? "Create Account →" : "Sign In to Portal →";
  ...
  const handleAuth = async (e: { preventDefault: () => void }) => {
    ...
    try {
      ...
      const { access_token } = await applicantLoginApi({ email, password, rememberMe });
        ...
        router.push(redirect?.startsWith("/applicant/") ? redirect : "/applicant/dashboard");
    } catch (err: any) {
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8 bg-card/30">
        <div className="w-full max-w-md space-y-10">
          ...
          <div className="flex p-1 bg-muted/60 rounded-xl border border-border">
            <button
              type="button"
              className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all ${isSignUp ? "text-muted-foreground hover:text-foreground" : "bg-background shadow-md text-primary"}`}
          ...
          <form onSubmit={handleAuth} className="space-y-5">
              ...
              <>
                <div className="flex gap-3">
                  <div className="space-y-2 flex-1">
                    <label htmlFor="applicant-first-name" className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1">First Name</label>
                    <Input
                  ...
                  <div className="space-y-2 flex-1">
                    <label htmlFor="applicant-last-name" className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1">Last Name</label>
                    <Input
                ...
                <div className="space-y-2">
                  <label htmlFor="applicant-phone-number" className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1">
            ...
            <div className="space-y-2">
              <label htmlFor="applicant-email-address" className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1">Email Address</label>
              <Input
            ...
            <div className="space-y-2">
              <label htmlFor="applicant-security-password" className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground ml-1">Security Password</label>
              <PasswordInput
                id="applicant-security-password"
                placeholder="••••••••"
                className="h-11 bg-background"
                value={password}
