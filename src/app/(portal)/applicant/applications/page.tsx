...
const STATUS_CONFIG: Record<string, { label: string; badge: string; dot: string; darkBadge: string }> = {
  ...
  withdrawn:           { label: "Withdrawn",           badge: "bg-slate-100 text-slate-700 border-slate-200",      dot: "bg-slate-500",    darkBadge: "bg-slate-500/20 text-slate-200 border-slate-400/30"      },
};

type SortKey = "date_desc" | "date_asc" | "status";
type FilterStatus = "all" | "active" | "hired" | "rejected" | "past";

function isTerminal(s: string) { return s === "hired" || s === "rejected" || s === "withdrawn" || s === "offer_accepted" || s === "offer_expired"; }
function isPastStatus(s: string) { return s === "offer_accepted" || s === "offer_expired" || s === "rejected" || s === "withdrawn"; }
function isActive(s: string)   { return !isPastStatus(s); }
...
function DetailModal({ detail, onClose, initialTab, onOfferAccepted, allApplications }: { readonly detail: DetailWithJob; readonly onClose: () => void; readonly initialTab?: "job" | "answers" | "interview"; readonly onOfferAccepted?: (appId: string) => void; readonly allApplications?: MyApplication[] }) {
          ...
          {tab === "job" && (
            <>
              <div className="flex flex-wrap gap-2">
                ...
                {/* Offer deadline */}
                {detail.offer_deadline && (localStatus === 'hired' || localStatus === 'offer_accepted') && (() => {
                  ...
                  return (
                    <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-semibold ${
                      ...
                      {daysLeft <= 0
                        ? `Offer expired on ${fmtDate(detail.offer_deadline)}`
                        : `Offer expires in ${daysLeft} day${daysLeft !== 1 ? 's' : ''} (${fmtDate(detail.offer_deadline)})`}
                    </div>
                  );
                })()}
              </div>
              ...
              <div>
                ...
                <div className="rounded-xl border border-border bg-muted/20 px-4 py-4">
                  ...
                  {localStatus === "hired" && (
                    (!detail.offer_deadline || new Date(detail.offer_deadline) >= new Date()) ? (
                      <button
                      ...
                      <p className="text-xs text-red-600 font-semibold mt-3">This offer has expired.</p>
                    )
                  )}
...
export default function ApplicantApplicationsPage() {
  ...
  const past     = applications.filter((a) => isPastStatus(a.status)).length;
  ...
  const filtered = useMemo(() => {
    ...
    if (filterStatus === "rejected") list = list.filter((a) => a.status === "rejected" || a.status === "withdrawn");
    if (filterStatus === "past")     list = list.filter((a) => isPastStatus(a.status));
    ...
    if (sort === "status") {
      const order = ["submitted","screening","first_interview","technical_interview","final_interview","hired","offer_accepted","offer_expired","rejected","withdrawn"];
      list.sort((a, b) => {
  ...
  const filterTabs: { key: FilterStatus; label: string; count: number; icon: any }[] = [
    ...
    { key: "past",     label: "Past",       count: past,      icon: PartyPopper },
  ];
        ...
        <DetailModal
          ...
          onOfferAccepted={(appId) => {
            setApplications((prev) =>
              prev.map((a) => {
                if (a.application_id === appId) return { ...a, status: "offer_accepted" };
                if (a.status === "hired") return { ...a, status: "offer_expired" };
                return a;
              })
            );
          }}
