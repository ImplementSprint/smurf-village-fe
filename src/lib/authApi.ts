...
// Strip any caller-supplied Authorization so authFetch can inject the current one
// cleanly. Without this, the Fetch spec's Headers ctor would *append* (combine
// with ", ") the stale and fresh values into a single malformed header instead
// of replacing it, and the backend would reject the request as "Invalid token".
function stripAuthorization(h: Record<string, string>): Record<string, string> {
...
export async function authFetch(input: RequestInfo, init: RequestInit = {}) {
  ...
  const first = await fetch(input, {
    ...
    credentials: "include",
    headers: {
      ...stripAuthorization(normalizeHeaders(init.headers)),
