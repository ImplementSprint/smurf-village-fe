...
import type { OnboardingSession, OnboardingSessionSummary, OnboardingTemplate, ProfileData, TemplateItem, TrainingVideo, TrainingVideoWithProgress } from '@/types/onboarding.types';
...
// ── Admin: Training Video Management ─────────────────────────────────────────

export async function adminUploadTrainingVideo(file: File): Promise<{ url: string; file_name: string; file_type: string; file_size: number }> {
  const formData = new FormData();
  formData.append('file', file);
  // No Content-Type header — the browser sets the multipart boundary.
  const res = await authFetch(`${API_BASE_URL}/onboarding/system-admin/training-videos/upload`, {
    method: 'POST',
    body: formData,
  });
  if (!res.ok) {
...
export async function adminGetTrainingVideos(templateId?: string): Promise<TrainingVideo[]> {
  const qs = templateId ? `?templateId=${encodeURIComponent(templateId)}` : '';
  const res = await authFetch(`${API_BASE_URL}/onboarding/system-admin/training-videos${qs}`);
  if (!res.ok) throw new Error('Failed to fetch training videos');
  return res.json();
}

export async function adminCreateTrainingVideo(payload: {
  template_id: string;
  title: string;
  description?: string;
  video_url: string;
  ...
  const res = await authFetch(`${API_BASE_URL}/onboarding/system-admin/training-videos`, {
    method: 'POST',
  ...
  if (!res.ok) {
    const body = await res.json().catch(() => ({}));
    throw new Error(body?.message || 'Failed to create training video');
  }
  return res.json();
}

export async function adminUpdateTrainingVideo(videoId: string, payload: Partial<{
  title: string;
  description: string;
  video_url: string;
  sequence_order: number;
  ...
  const res = await authFetch(`${API_BASE_URL}/onboarding/system-admin/training-videos/${videoId}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload),
  });
  if (!res.ok) {
    const body = await res.json().catch(() => ({}));
    throw new Error(body?.message || 'Failed to update training video');
  }
  return res.json();
}
...
export async function adminDeleteTrainingVideo(videoId: string): Promise<void> {
  ...
  if (!res.ok) throw new Error('Failed to delete training video');
}

// ── Applicant Portal: Training Videos ────────────────────────────────────────

export async function getMyTrainingVideos(): Promise<TrainingVideoWithProgress[]> {
  const base = onboardingBase();
  const res = await fetch(`${base}/training-videos`, { headers: headers() });
  if (!res.ok) throw new Error('Failed to fetch training videos');
  return res.json();
}

export async function saveVideoProgress(videoId: string, payload: {
  watched_seconds: number;
  max_watched_seconds: number;
  is_completed?: boolean;
}): Promise<any> {
  const base = onboardingBase();
  const res = await fetch(`${base}/training-videos/${videoId}/progress`, {
    method: 'POST',
    headers: headers(),
    body: JSON.stringify(payload),
  });
