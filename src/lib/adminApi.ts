...
export async function setUserStatus(userId: string, status: "active" | "locked"): Promise<void> {
  throw new Error(`setUserStatus is not implemented yet for ${userId}:${status}`);
}
...
export async function updateUser(
  ...
  throw new Error(`updateUser is not implemented yet for ${userId}:${Object.keys(payload).join(",")}`);
}
...
export async function updateSubscription(
  ...
  throw new Error(`updateSubscription is not implemented yet for ${subscriptionId}:${Object.keys(payload).join(",")}`);
}
