...
import { RoleCheckboxGroup } from "@/components/employees/RoleCheckboxGroup";
...
export function EmployeeProfileSheet({
  ...
  const [employmentDraft, setEmploymentDraft] = useState({
    ...
    role_ids: (initialEmployee.role_ids && initialEmployee.role_ids.length > 0)
      ? initialEmployee.role_ids
      : (initialEmployee.role_id ? [initialEmployee.role_id] : []),
    ...
    apiFetch<EmployeeRecord>(`/users/${initialEmployee.user_id}`)
      .then((data) => {
        ...
        setEmploymentDraft({
          ...
          role_ids: (data.role_ids && data.role_ids.length > 0)
            ? data.role_ids
  ...
  const assignedIds = profile.role_ids && profile.role_ids.length > 0
    ? profile.role_ids
    : (profile.role_id ? [profile.role_id] : []);
  const roleNames = assignedIds.map(id => roles.find(r => r.role_id === id)?.role_name).filter(Boolean) as string[];
    ...
    try {
      const updated = await apiFetch<EmployeeRecord>(`/users/${profile.user_id}`, {
        ...
        body: JSON.stringify({
          ...
          role_ids: employmentDraft.role_ids,
        ...
        <div className="relative ml-auto h-full w-full max-w-2xl bg-background shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
          ...
          <div className="shrink-0 bg-[linear-gradient(135deg,#0f172a_0%,#172554_60%,#134e4a_100%)] text-white px-6 py-6">
            ...
            <div className="flex items-center gap-3 mt-5 flex-wrap">
              ...
              {/* Role chips — one per assigned role */}
              {roleNames.length === 0 ? (
                <div className="flex items-center gap-1.5 bg-white/10 rounded-full px-3 py-1 text-xs font-medium text-white/85 border border-white/15">
                ...
                {activeTab === "overview" && (
                  <div className="space-y-6">
                    ...
                    <div className="bg-card border border-border rounded-xl p-5">
                      ...
                      <div className="space-y-0.5">
                        {roleNames.length <= 1 ? (
                          <InfoRow icon={Shield} label="Role" value={roleNames[0] ?? "—"} />
                        ) : (
                          <div className="flex items-start gap-3 px-1 py-2.5 border-b border-border/40 last:border-0">
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="bg-card border border-border rounded-xl p-5">
                      <SectionHeader
                        ...
                        onCancel={() => { setEditingEmployment(false); setEmploymentDraft({ first_name: profile.first_name, middle_name: profile.middle_name ?? "", last_name: profile.last_name, role_ids: (profile.role_ids && profile.role_ids.length > 0) ? profile.role_ids : (profile.role_id ? [profile.role_id] : []), department_id: profile.department_id ?? "", start_date: profile.start_date ?? "" }); }}
                      />
                      ...
                      <div className="grid sm:grid-cols-2 gap-3 mb-4">
                        <div className="space-y-1.5">
                          <label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                            Role {editingEmployment && <span className="normal-case font-normal text-muted-foreground/70">(select one or more)</span>}
                          </label>
                          {editingEmployment ? (
                            <RoleCheckboxGroup
                              roles={roles}
                              selectedIds={employmentDraft.role_ids}
                              onChange={ids => setEmploymentDraft(d => ({ ...d, role_ids: ids }))}
                            />
                          ) : (
      ...
      {confirmEmployment && (
        <ConfirmEditDialog
          fields={[
            ...
            { label: "Role",        before: roleNames.join(", "),  after: employmentDraft.role_ids.map(id => roles.find(r => r.role_id === id)?.role_name).filter(Boolean).join(", ") },
