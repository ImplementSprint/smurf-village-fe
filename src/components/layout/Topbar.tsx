...
import { ArrowLeftRight, Bell, ChevronDown, Loader2, LogOut, User } from "lucide-react";
...
export function Topbar({ persona = "applicant" }: { readonly persona?: PersonaType }) {
  ...
  return (
    <header className="h-16 bg-background border-b border-border flex items-center justify-end px-8 shrink-0">
      <div className="flex items-center gap-6">

        {/* Switch Role — only for multi-portal accounts */}
        {(user?.available_portals?.length ?? 0) > 1 && persona !== "applicant" && (
          <button
            onClick={() => router.push("/portal-select")}
            className="flex items-center gap-1.5 text-xs font-medium text-muted-foreground hover:text-primary hover:bg-primary/5 px-3 py-1.5 rounded-lg border border-border hover:border-primary/30 transition-all"
          >
