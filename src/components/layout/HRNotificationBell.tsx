...
export function HRNotificationBell() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([interviews, allNotifs]) => {
        if (!alive) return;
        setItems(interviews);
        setReadIds(loadReadIds(userEmail));
  ...
  if (loading) {
    ...
    const responseSuffix = unreadCount > 1 ? "s" : "";
    headerSummary = `${unreadCount} new response${responseSuffix}`;
  }
  ...
  const markDbNotifAsRead = useCallback((notificationId: string) => {
    setDbNotifs((prev) => prev.map((n) => (
      n.notification_id === notificationId ? { ...n, is_read: true } : n
    )));
  }, []);

  const handleDbNotificationClick = useCallback(async (notification: AppNotification) => {
    if (!notification.is_read) {
      await markNotificationRead(notification.notification_id);
      markDbNotifAsRead(notification.notification_id);
    }
    setOpen(false);
    router.push('/hr/onboarding');
  }, [markDbNotifAsRead, router]);
  ...
  } else if (items.length === 0 && dbNotifs.length === 0) {
    dropdownBody = (
            ...
            {dbNotifs.map((notif) => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    void handleDbNotificationClick(notif);
                  }}
                  className={`w-full flex items-start gap-3 px-4 py-3.5 text-left transition-colors cursor-pointer group ${
