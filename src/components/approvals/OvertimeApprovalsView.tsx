...
export default function OvertimeApprovalsView() {
  ...
  const pendingCount = requests.filter((r) => r.log_status === "PENDING").length;
  ...
  return (
    <>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base font-bold tracking-tight flex items-center gap-2">
              ...
              Overtime Requests
              {pendingCount > 0 && (
                <span className="ml-auto inline-flex items-center justify-center rounded-full bg-amber-500 text-white text-[10px] font-bold h-5 min-w-5 px-1.5">
                  {pendingCount}
                </span>
              )}
            </CardTitle>
              ...
              <div className="space-y-3">
                {requests.map((req) => {
                  ...
                  return (
                    <div
                      key={req.ot_id}
                      className="rounded-lg border p-4 bg-muted/10 space-y-3 hover:bg-primary/5 transition-colors"
                    >
                      <div className="flex flex-col gap-1 md:flex-row md:items-start md:justify-between">
                        ...
                        <div className="flex gap-2">
                          <Badge className={typeColor(req.ot_type)}>
                            {OT_TYPE_LABELS[req.ot_type]}
                          </Badge>
                      ...
                      {isPending && (
                        <div className="flex flex-wrap gap-2">
                          <Button
                            ...
                            onClick={() => void handleApprove(req.ot_id)}
                          >
                            {isProcessing ? (
      ...
      <Dialog open={!!denyDialogId} onOpenChange={(open) => {
        if (!open) setDenyDialogId(null);
      }}>
        <DialogContent>
          <DialogHeader>
          ...
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Provide a reason for denial (required, min 3 chars).
            </p>
            <Textarea
