...
export function VideoTrainingPlayer({ onCompletionChange }: Readonly<VideoTrainingPlayerProps> = {}) {
  ...
  // Notify parent whenever video completion state changes — never inside a state setter
  useEffect(() => {
    if (!loading && videos.length > 0) {
      onCompletionChange?.(videos.every((v) => v.progress?.is_completed));
    }
  }, [videos, loading, onCompletionChange]);
