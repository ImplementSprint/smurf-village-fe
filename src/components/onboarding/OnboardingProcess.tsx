...
import {
  ...
  Briefcase, CalendarDays, ChevronRight, Video,
} from "lucide-react";
...
export function OnboardingProcess({
  ...
  const [trainingAllCompleted, setTrainingAllCompleted] = useState(false);
  ...
  const trainingUnlocked  = equipmentUnlocked && equipmentAllApproved;
  const finalReady = progress === 100 && equipmentAllApproved && trainingAllCompleted;
  ...
  const handleSubmitForReview = async () => {
    if (!finalReady) {
      ...
      if (!trainingAllCompleted) {
        alert("Please finish all required training videos before submitting for final review.");
        return;
      }
  ...
  // Training is the final step — only unlocked after all prior steps are HR-approved
  const steps = [
    ...
    { value: "training",  label: "Training",   icon: Video,        unlocked: trainingUnlocked,   approved: trainingAllCompleted },
  ] as const;
  ...
  return (
    <div className="space-y-5">
        ...
        <Tabs defaultValue="profile" className="w-full">
          ...
          <div className="p-5">
            ...
            <TabsContent value="training" className="mt-0">
              {!trainingUnlocked ? (
                <LockedStep label="Equipment" />
              ) : (
                <div className="px-6 py-5">
                  <VideoTrainingPlayer onCompletionChange={setTrainingAllCompleted} />
                </div>
              )}
            </TabsContent>
          </div>
      ...
      {progress === 100 && equipmentAllApproved && !trainingAllCompleted && session.status !== "for-review" && session.status !== "approved" && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-5 py-4 flex items-center gap-3">
          <AlertCircle className="size-4 text-amber-600 shrink-0" />
          <p className="text-xs text-amber-800">
            Please watch all <strong>Training Videos</strong> to completion before submitting for final HR review.
          </p>
        </div>
      )}
